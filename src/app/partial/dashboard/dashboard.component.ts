...
export class DashboardComponent implements OnInit {
  ...
  getOverSpeedData() {
    ...
    this.alertTypeArray.subscribe((x:any)=>{
      ...
      this.apiCall.setHttp('get', 'dashboard/get-vehicle-alert-count-dashboard?UserId=' + this.webStorage.getUserId() + '&Fromdate=' + Fromdate + '&ToDate=' + ToDate+'&alertType='+x, true, false, false, 'vehicletrackingBaseUrlApi');
      this.apiCall.getHttp().subscribe((responseData: any) => {
        if (responseData.statusCode === "200" || responseData.length > 0) {
